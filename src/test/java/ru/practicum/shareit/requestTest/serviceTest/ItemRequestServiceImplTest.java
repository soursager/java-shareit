...
public class ItemRequestServiceImplTest {
   ...
   private UserValidatorService userValidator;
    ...
    @Mock
   private RequestValidatorService itemRequestValidator;

    @InjectMocks
   private ItemRequestServicelmpl requestService;
    ...
    void getAllUserRequestsWithResponses_whenUserExists_thenReturnListOfResponses() {
        doNothing().when(userValidator).checkingUserIdAndNotReturn(anyLong());
    ...
    void getAllUserRequestsWithResponses_whenUserNotExists_thenThrowEntityNotFoundException() {
        doThrow(new DataNotFoundException("Пользователь не существует!"))
                .when(userValidator).checkingUserIdAndNotReturn(anyLong());
    ...
    void getRequestById_whenUserAndRequestExist_thenReturnRequest() {
        ...
        doNothing().when(userValidator).checkingUserIdAndNotReturn(anyLong());
        when(itemRequestValidator.validateItemRequestIdAndReturn(anyLong())).thenReturn(new ItemRequest());
    ...
    void getRequestById_whenRequestNotFound_thenThrowEntityNotFoundException() {
        doNothing().when(userValidator).checkingUserIdAndNotReturn(anyLong());
        when(itemRequestValidator.validateItemRequestIdAndReturn(anyLong()))
